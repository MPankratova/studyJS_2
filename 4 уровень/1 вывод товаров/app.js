...
let colBtn = document.querySelectorAll('button');
let viewProduct = document.querySelector('.products');
colBtn.forEach(btn => btn.addEventListener('click', clickHandler));
...
function showCategory(category) {
    for (let i = 0; i < products[category].length; i++) {
        let item = getProductMarkup(products[category][i]);
        viewProduct.insertAdjacentHTML('afterbegin', item);
    }   
}
